  ...
  with HasPerfEvents
{
  ...
  val backendIntFlStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && dispatch.io.enqRob.canAccept && AllDqCanAccept && !rename.io.intFlCanAccept && rename.io.fpFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendFpFlStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && dispatch.io.enqRob.canAccept && AllDqCanAccept && rename.io.intFlCanAccept && !rename.io.fpFlCanAccept && !rename.io.rabCommits.isWalk})  )
  ...
  val backendintFlAndRobStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && AllDqCanAccept && !rename.io.intFlCanAccept && rename.io.fpFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendfpFlAndRobStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && AllDqCanAccept && rename.io.intFlCanAccept && !rename.io.fpFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendintDqAndintFlStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && !intDqCanAccept && fpDq.io.enq.canAccept && lsDq.io.enq.canAccept && !rename.io.intFlCanAccept && rename.io.fpFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendlsDqAndintFlStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && intDqCanAccept && fpDq.io.enq.canAccept && !lsDq.io.enq.canAccept && !rename.io.intFlCanAccept && rename.io.fpFlCanAccept && !rename.io.rabCommits.isWalk})  )
