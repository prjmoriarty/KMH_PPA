  ...
  with HasPerfEvents
{
  ...
  // Topdown
  val frontendStall = PopCount( rename.io.in map { ren => !(ren.valid || CommitType.isFused(ren.bits.commitType)) &&  ren.ready })
  val backendStall  = PopCount( rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready })
  val bidirectStall = PopCount( rename.io.in map { ren => !(ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready })
  val opSpec        = PopCount( rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) &&  ren.ready })
  ...
  val backendRobStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && AllDqCanAccept && AllFlCanAccept && !rename.io.rabCommits.isWalk }) )
  val backendWalkStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && dispatch.io.enqRob.canAccept && AllDqCanAccept && AllFlCanAccept && rename.io.rabCommits.isWalk }) )
  val backendIntFlStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && dispatch.io.enqRob.canAccept && AllDqCanAccept && !rename.io.intFlCanAccept && rename.io.intFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendFpFlStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && dispatch.io.enqRob.canAccept && AllDqCanAccept && rename.io.intFlCanAccept && !rename.io.intFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendIntDqStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && dispatch.io.enqRob.canAccept && !intDqCanAccept && fpDq.io.enq.canAccept && lsDq.io.enq.canAccept && AllFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendFpDqStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && dispatch.io.enqRob.canAccept && intDqCanAccept && !fpDq.io.enq.canAccept && lsDq.io.enq.canAccept && AllFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendLsDqStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && dispatch.io.enqRob.canAccept && intDqCanAccept && fpDq.io.enq.canAccept && !lsDq.io.enq.canAccept && AllFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendintFlAndRobStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && AllDqCanAccept && !rename.io.intFlCanAccept && rename.io.intFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendfpFlAndRobStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && AllDqCanAccept && rename.io.intFlCanAccept && !rename.io.intFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendintDqAndintFlStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && !intDqCanAccept && fpDq.io.enq.canAccept && lsDq.io.enq.canAccept && !rename.io.intFlCanAccept && rename.io.intFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendlsDqAndintFlStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && intDqCanAccept && fpDq.io.enq.canAccept && !lsDq.io.enq.canAccept && !rename.io.intFlCanAccept && rename.io.intFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendRobAndintDqStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && !intDqCanAccept && fpDq.io.enq.canAccept && lsDq.io.enq.canAccept && AllFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendRobAndlsDqStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && intDqCanAccept && fpDq.io.enq.canAccept && !lsDq.io.enq.canAccept && AllFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendRobAndfpDqStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && intDqCanAccept && !fpDq.io.enq.canAccept && lsDq.io.enq.canAccept&& AllFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendRobAndintlsDqStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && !dispatch.io.enqRob.canAccept && !intDqCanAccept && fpDq.io.enq.canAccept && !lsDq.io.enq.canAccept && AllFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendintAndlsDqStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.commitType)) && !ren.ready && dispatch.io.enqRob.canAccept && !intDqCanAccept && fpDq.io.enq.canAccept && !lsDq.io.enq.canAccept && AllFlCanAccept && !rename.io.rabCommits.isWalk})  )
